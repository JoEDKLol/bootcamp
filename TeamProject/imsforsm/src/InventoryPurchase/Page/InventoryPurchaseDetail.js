...
const InventoryPurchaseDetail = () => {
    ...
    const [purchaseDataInPeriod, setPuurchaseDataInPeriod] = useState();
    ...
    const purchaseData = companyPurchaseData.filter(data => data.action === 'Purchase')
    ...
    const clickHandler = (e) => {
        ...
        const purchaseDataPeriod = companyPurchaseData.filter(data => {
            return data.action === 'Purchase' && data.date.slice(0, 10) >= newStartDate && data.date.slice(0, 10) <= newEndDate
        });
        setPuurchaseDataInPeriod(purchaseDataPeriod);
    ...
    const selectHandler = e => {
        ...
        navigate(`/purchaseDetail/${e.target.value}`)
        setSearchMode(false);
        setStartDate(null);
        setEndDate(null);
    ...
    return (
        <div>
            ...
            <div className='content-box table-box'>
                {!SearchMode && <Table tableHead={tableHead} tableData={purchaseData} />}
                {SearchMode && <Table tableHead={tableHead} tableData={purchaseDataInPeriod} />}
            </div>
        </div >
    )
